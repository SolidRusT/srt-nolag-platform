...
{
    [Info("Monument Plus", "Ts3Hosting", "1.0.30", ResourceId = 2)]
    ...
    public class MonumentPlus : RustPlugin
    {
        ...
        public Dictionary<CardReader, TimerSwitch> newPuzzles2 = new Dictionary<CardReader, TimerSwitch>();
        ...
        private static List<vectorPos> GasStationDoor
        {
            get
            {
                return new List<vectorPos>
                {
                    new vectorPos { pos = new Vector3(12.02f, 3.15f, 23.4f), rot = 90f, prefab = "assets/content/structures/interactive_garage_door/sliding_blast_door.prefab", moveRotate = 90f },
                    new vectorPos { pos = new Vector3(12.02f, 3.15f, 27.61f), rot = 90f, prefab = "assets/content/structures/interactive_garage_door/sliding_blast_door.prefab", moveRotate = 90f },
                    new vectorPos { pos = new Vector3(-10.61f, 3.342f, 22.25f), rot = -90f, prefab = "assets/prefabs/io/electric/switches/cardreader.prefab", moveRotate = 0f },
                    new vectorPos { pos = new Vector3(13.42f, 1.142f, 28.4f), rot = 0f, prefab = "assets/prefabs/deployable/playerioents/splitter/splitter.prefab", moveRotate = 90f },
                    new vectorPos { pos = new Vector3(11.91f, -4.0f, 20.4f), rot = 90f, prefab = "assets/prefabs/deployable/playerioents/timers/timer.prefab", moveRotate = 9f },
                    new vectorPos { pos = new Vector3(11.90f, 2.942f, 25.4f), rot = 90f, prefab = "assets/prefabs/io/electric/switches/fusebox/fusebox.prefab", moveRotate = 0f }
                };
            }
        }

        private static List<vectorPos> GasStationBoxLoc
        {
            get
            {
                return new List<vectorPos>
                {
                    new vectorPos { pos = new Vector3(12.82f, 1.918f, 27.35f), rot = 0f, prefab = "assets/bundled/prefabs/radtown/crate_normal_2.prefab", moveRotate = 0f },
                    new vectorPos { pos = new Vector3(14.22f, 2.0f, 23.50f), rot = 0f, prefab = "assets/bundled/prefabs/radtown/crate_normal.prefab", moveRotate = 0f }
                };
            }
        }

        #endregion GasstationLocations
        ...
        #endregion SupermarketLocations
        ...
        #endregion MiningOutpostLocations
        ...
        {
            if (newPuzzles.ContainsKey(reader1) && !newPuzzles2.ContainsKey(reader1))
            {
            ...
            else if (newPuzzles2.ContainsKey(reader1))
            {
                if (newPuzzles2[reader1] != null)
                    newPuzzles2[reader1].UpdateFromInput(25, 1);
                timer.Once(0.2f, () => { if (newPuzzles2[reader1] != null) newPuzzles2[reader1].UpdateFromInput(0, 1); });
            }
        ...
        private void connectWires(IOEntity entity, IOEntity entity1, int output = 0, int input = 0)
        {
            IOEntity.IOSlot ioOutput = entity.outputs[output];
            if (ioOutput != null)
            {
                ioOutput.connectedTo = new IOEntity.IORef();
                ioOutput.connectedTo.Set(entity1);
                ioOutput.connectedToSlot = input;
                ioOutput.connectedTo.Init();

                entity1.inputs[input].connectedTo = new IOEntity.IORef();
                entity1.inputs[input].connectedTo.Set(entity);
                entity1.inputs[input].connectedToSlot = output;
                entity1.inputs[input].connectedTo.Init();
        ...
        BaseEntity SpawnTheEntitys(Vector3 pos, float rot, float rotation, string prefab, MonumentInfo monument)
        {
            ...
            //	if (itemsEntity is SlidingProgressDoor)
            //		(itemsEntity as SlidingProgressDoor).openPosition.y = 5f;
        ...
        private void CheckTheEntitys(IOEntity reader1, DoorManipulator controler, IOEntity button1, IOEntity theBox, bool justConnect = false, int output = 0, int input = 0)
        {
            if (justConnect)
            {
                if (reader1 != null && theBox != null)
                {
                    connectWires(theBox, reader1, output, input);
                    if (theBox is FuseBox || theBox is LaserDetector)
                        theBox.UpdateFromInput(250, 0);
                }
                return;
            }
            ...
            foreach (var monument in UnityEngine.Object.FindObjectsOfType<MonumentInfo>())
            {
                ...
                switch (monumentName)
                {
                    ...
                    case MonumentName.GasStation:
                        {
                            if (!configData.settings.GasStation || unLoading) continue;

                            IOEntity door1 = null;
                            IOEntity door2 = null;
                            IOEntity spliter = null;
                            IOEntity button = null;
                            IOEntity reader1 = null;
                            IOEntity timer = null;
                            IOEntity theBox = null;
                            IOEntity lazer = null;
                            IOEntity lazer1 = null;
                            IOEntity lazer2 = null;
                            IOEntity lazer3 = null;
                            IOEntity lazer4 = null;
                            IOEntity lazer5 = null;
                            blastDoors theMono = null;
                            blastDoors theMono2 = null;

                            foreach (var key in GasStationDoor)
                            {
                                itemsVector = monument.transform.TransformPoint(key.pos);
                                if (itemsVector != Vector3.zero)
                                {
                                    if (key.prefab.Contains("sliding_blast"))
                                        RemoveOldDoor(itemsVector, 0.0001f);
                                    ...
                                    if (itemsEntity is SlidingProgressDoor)
                                    {
                                        if (door1 == null)
                                        {
                                            door1 = itemsEntity as IOEntity;
                                            theMono2 = (itemsEntity as SlidingProgressDoor).gameObject.AddComponent<blastDoors>();
                                        }
                                        else
                                        {
                                            door2 = itemsEntity as IOEntity;
                                            theMono = (itemsEntity as SlidingProgressDoor).gameObject.AddComponent<blastDoors>();
                                        }
                                    }
                                    else if (itemsEntity is LaserDetector)
                                    {
                                        if (lazer == null)
                                            lazer = itemsEntity as IOEntity;
                                        else if (lazer1 == null)
                                            lazer1 = itemsEntity as IOEntity;
                                        else if (lazer2 == null)
                                            lazer2 = itemsEntity as IOEntity;
                                        else if (lazer3 == null)
                                            lazer3 = itemsEntity as IOEntity;
                                        else if (lazer4 == null)
                                            lazer4 = itemsEntity as IOEntity;
                                        else if (lazer5 == null)
                                            lazer5 = itemsEntity as IOEntity;
                                    }
                                    else if (itemsEntity is CardReader)
                                    {
                                        reader1 = itemsEntity as IOEntity;
                                        (itemsEntity as CardReader).accessDuration = 0.2f;
                                    }
                                    else if (itemsEntity is Splitter)
                                    {
                                        spliter = itemsEntity as IOEntity;
                                    }
                                    else if (itemsEntity is ItemBasedFlowRestrictor)
                                    {
                                        theBox = itemsEntity as IOEntity;
                                    }
                                    else if (itemsEntity is TimerSwitch)
                                    {
                                        timer = itemsEntity as IOEntity;
                                        (itemsEntity as TimerSwitch).timerLength = 20f;
                                    }
                                }
                            }
                            if (theMono != null) theMono.theBox = timer;
                            if (theMono2 != null) theMono2.theBox = timer;

                            CheckTheEntitys(reader1, null, null, theBox, true);
                            theBox.UpdateFromInput(250, 0);
                            CheckTheEntitys(spliter, null, null, timer, true);
                            timer.UpdateFromInput(250, 0);
                            CheckTheEntitys(door1, null, null, spliter, true);
                            CheckTheEntitys(door2, null, null, spliter, true, 1);
                         //   CheckTheEntitys(timer, null, null, reader1, true, 0, 1);

                            newPuzzles2.Add((reader1 as CardReader), (timer as TimerSwitch));

                            foreach (var key in GasStationBoxLoc)
                            {
                                itemsVector = monument.transform.TransformPoint(key.pos);
                                if (itemsVector != Vector3.zero)
                                {
        ...
        public class blastDoors : FacepunchBehaviour
        {
            public SlidingProgressDoor door { get; set; }
            private doorColider controler { get; set; }
            private GameObject objects { get; set; }
            public IOEntity theBox { get; set; }
            private float nextUpdate { get; set; }
            public BoxCollider boxCollider { get; set; }

            private void Awake()
            {
                door = GetComponent<SlidingProgressDoor>();
                controler = new GameObject().AddComponent<doorColider>();
                if (controler != null)
                {
                    controler.setup(this);
                    boxCollider = controler.boxCollider;
                }
            }

            public void OnTriggerEnterNew(Collider col)
            {
                ...
                {
                    if (theBox == null || (baseEntity as BasePlayer) == null || (baseEntity as BasePlayer).IsSleeping() || (baseEntity as BasePlayer).IsDead())
                        return;
                  //  drawZones((baseEntity as BasePlayer), controler.boxCollider.transform.position, controler.boxCollider.transform.rotation.y, controler.boxCollider.size / 2);
                    theBox.UpdateFromInput(250, 1);
                    _instance.timer.In(0.5f, () => theBox.UpdateFromInput(0, 1));
                }
            }
            ...
            private void Update()
            {
                if (controler != null && boxCollider != null)
                {
                    if (door.IsPowered())
                    {
                        ...
                        {
                            var inputAmount = 0.5f;
                            door.AddEnergy(inputAmount);
                            door.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
                            nextUpdate = Time.time + 0.1f;
                        }
                    }
                    else if (boxCollider.isTrigger == false)
                    {
        ...
        public class doorColider : MonoBehaviour
        {
            ...
            public BoxCollider boxCollider { get; set; }
            public blastDoors door { get; set; }
            ...
            public void setup(blastDoors entity)
            {
                door = entity;
                ...
                boxCollider.transform.position = new Vector3(door.door.transform.position.x, door.door.transform.position.y - 0.8f, door.door.transform.position.z);
                boxCollider.transform.rotation = Quaternion.Euler(0f, door.door.transform.rotation.y, 0f);
                boxCollider.isTrigger = true;
                boxCollider.gameObject.layer = 3;
                boxCollider.gameObject.name = "Trigger";
                boxCollider.size = new Vector3(8.0f, 1.0f, 8.0f);
