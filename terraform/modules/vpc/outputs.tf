...
output nat_gw_ip_1 {
  value = var.nat_gw ? join("", aws_eip.nat_1.*.public_ip) : ""
}

output nat_gw_ip_2 {
  value = var.nat_gw_multi_az ? join("", aws_eip.nat_2.*.public_ip) : ""
}

output nat_instance_ip_1 {
  value = var.nat_instance ? join("", aws_instance.nat_1.*.public_ip) : ""
}

output nat_instance_ip_2 {
  value = var.nat_instance_multi_az ? join("", aws_instance.nat_2.*.public_ip) : ""
}
