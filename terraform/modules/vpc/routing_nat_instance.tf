...
resource "aws_route_table" "nat_instance_1" {
  ...
  route {
    ...
    instance_id = aws_instance.nat_1[count.index].id
  }

  tags = merge(var.common_tags, map(
    "Name", "${var.project}-nat-box-1-${var.env}"
  ))
}
resource "aws_route_table_association" "app_1_subnet_to_nat_instance" {
  count          = var.nat_instance ? 1 : 0
  route_table_id = aws_route_table.nat_instance_1[count.index].id
...
resource "aws_route_table" "nat_instance_2" {
  ...
  route {
    cidr_block  = "0.0.0.0/0"
    instance_id = aws_nat_gateway.nat_2[count.index].id
  }

  tags = merge(var.common_tags, map(
    "Name", "${var.project}-nat-box-2-${var.env}"
  ))
}
resource "aws_route_table_association" "app_2_subnet_to_nat_instance" {
  count          = var.nat_instance_multi_az ? 1 : 0
  route_table_id = aws_route_table.nat_instance_2[count.index].id
