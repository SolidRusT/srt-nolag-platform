...
{
    [Info("Gather Rewards", "CanopySheep", "1.5.4", ResourceId = 770)]
    ...
    public class GatherRewards : RustPlugin
    {
        ...
        PluginConfig DefaultConfig()
        {
            ...
            foreach (GameManifest.PooledString str in GameManifest.Current.pooledStrings)
            {
                if (str.str.StartsWith("assets/rust.ai/agents"))
                {
					...
					if (str.str.Contains("test")) { continue; }
                    if (str.str.Contains("npc")) { continue; }
        ...
        private void OnDispenserGather(ResourceDispenser dispenser, BaseEntity entity, Item item)
        {
            ...
            if (player)
            {
                ...
                {
                    if (shortName.Contains(configValue.Key.ToLower()))
                    {
		...
		private void OnCollectiblePickup(Item item, BasePlayer player)
		{
            ...
            foreach (KeyValuePair<string, float> configValue in config.Rewards)
            {
                if (item.ToString().ToLower().Contains(configValue.Key.ToLower()))
                {
                    amount = config.Rewards[configValue.Key];
                    resource = configValue.Key;
                    break;
                }
            }
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity.ToPlayer() != null && !(entity is NPCPlayer))
            {
                ...
                amount = config.Rewards[PluginRewards.Player];
                animal = "player";
                if (Friends && config.Rewards[PluginRewards.PlayerFriend] != 0)
                {
                    ...
                    {
                        amount = config.Rewards[PluginRewards.PlayerFriend];
                        animal = "friend";
                    }
                }
                if (Clans && config.Rewards[PluginRewards.ClanMember] != 0)
                {
                    ...
                    {
                        amount = config.Rewards[PluginRewards.ClanMember];
                        animal = "clan member";
                    }
                }
            }
            ...
            {
                if (entity is NPCMurderer) { animal = "Murderer"; config.Rewards.TryGetValue(animal, out amount); }
				else if (entity is NPCPlayer && (entity as BaseEntity).GetType().ToString().Contains("Scientist")) 
                { 
                    animal = "Scientist"; config.Rewards.TryGetValue(animal, out amount); 
                }
                else if (entity.GetComponent("BaseNpc"))
				{
					animal = UppercaseFirst(entity.LookupPrefab().ToString().Replace("[0]", ""));
                    Puts(animal);
					config.Rewards.TryGetValue(animal, out amount);
				}
            }
        ...
        private void cmdGatherRewards(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                {
                    bool found = false;
                    foreach (KeyValuePair<string, float> entry in config.Rewards)
                    {
                        if (found) { continue; }
