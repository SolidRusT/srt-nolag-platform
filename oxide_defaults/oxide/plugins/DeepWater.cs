...
{
    [Info("DeepWater", "Colon Blow", "1.0.26")]
    class DeepWater : RustPlugin
    {
        // fix for ch47 flying off to rust oil rigs
        // fix for grid callout not correct
        // fix for config not showing default values after deleting and reloading plugin
        ...
        public class DeepWaterEntity : MonoBehaviour
        {
            ...
            void DeepWaterStage1()
            {
                ...
                if (getai != null)
                {
                    getai.SetLandingTarget(helilogo1.transform.position);
                    getai.hoverHeight = -10f;
                }
                helicopter?.Spawn();
                RefreshAll();
            }
