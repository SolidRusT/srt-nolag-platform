...
{
    [Info("TrainRustEdit", "KpucTaJl", "1.0.3")]
    class TrainRustEdit : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            foreach (var dic in trainsInit)
            {
                if (dic.Key != null) dic.Key.Kill();
                NextTick(() => SpawnTrain(dic.Value.pos.ToVector3(), dic.Value.rot.ToVector3()));
            }
        }
        ...
        public class TrainController : FacepunchBehaviour
        {
            ...
            void FixedUpdate()
            {
                ...
                if (ins.TriggerChangePoints.Any(x => Vector3.Distance(train.transform.position, x.ToVector3()) < 25f)) foreach (TriggerTrainCollisions trigger in triggers) trigger.triggerCollider.gameObject.layer = 21;
