...
{
    [Info("Night Zombies", "0x89A", "2.1.3")]
    ...
    class NightZombies : CovalencePlugin
    {
        ...
        private bool IsDay => Env.time >= TOD_Sky.Instance.SunriseTime && Env.time < TOD_Sky.Instance.SunsetTime;
        private bool IsNight => Env.time >= TOD_Sky.Instance.SunsetTime || Env.time < TOD_Sky.Instance.SunriseTime;
        ...
        {
            if ((config.SpawnSettings.reverseTimings && IsDay || !config.SpawnSettings.reverseTimings && IsNight) && daysSinceSpawn >= config.SpawnSettings.Chance.days && Random.Range(0f, 100f) <= config.SpawnSettings.Chance.chance) return true;
            else return false;
        }
