...
{
    [Info("SkinBox", "k1lly0u", "2.1.2"), Description("Allows you to reskin item's by placing it in the SkinBox and selecting a new skin")]
    class SkinBox : RustPlugin
    {
        ...
        private void CollectApprovedSkins()
        {
            ...
            foreach (ItemDefinition itemDefinition in ItemManager.itemList)
            {
                ...
                foreach (Steamworks.InventoryDef item in Steamworks.SteamInventory.Definitions)
                {
                    ...
                    if (string.IsNullOrEmpty(item.Name) || skinId == 13059 || skinId == 13058)
                    {
                        Debug.Log($"{skinId} {shortname} {item.Id}");
                    }
        ...
        private class DeployableHandler : LootHandler
        {
            ...
            internal override void OnItemRemoved(Item item)
            {
                ...
                wasSuccess = ReskinEntity(reskinTarget.entity, reskinTarget.itemDefinition, item);
            ...
            private bool ReskinEntity(BaseEntity targetEntity, ItemDefinition defaultDefinition, Item targetItem)
            {
                string reason;
                ...
                if (defaultDefinition != targetItem.info)
                    return ChangeSkinForRedirectedItem(targetEntity, targetItem);
                return ChangeSkinForExistingItem(targetEntity, targetItem);
            }

            private bool ChangeSkinForRedirectedItem(BaseEntity targetEntity, Item targetItem)
            {
                string resourcePath;
            ...
            private bool ChangeSkinForExistingItem(BaseEntity targetEntity, Item targetItem)
            {
            ...
            private int GetEntitySlot(BaseEntity rootEntity, BaseEntity childEntity)
            {
            ...
            private bool GetEntityPrefabPath(ItemDefinition itemDefinition, out string resourcePath)
            {
            ...
            private void SaveEntityStorage(BaseEntity baseEntity, Dictionary<ContainerSet, List<Item>> dictionary, int index)
            {
            ...
            private void RestoreEntityStorage(BaseEntity baseEntity, int index, Dictionary<ContainerSet, List<Item>> copy)
            {
                IItemContainerEntity itemContainerEntity = baseEntity as IItemContainerEntity;
                if (itemContainerEntity != null)
                {
            ...
            private void RestoreRigidbody(Rigidbody rb)
            {
                if (rb != null)                
                    rb.useGravity = true;                
            }
