...
namespace Oxide.Plugins
{
    ...
    public class Duelist : RustPlugin
    {
        ...
        [PluginReference] private Plugin Kits, ZoneManager, Economics, ServerRewards, LustyMap, Backpacks, PermaMap, Clans, AimTrain, PreferredEnvironment;
        ...
        class Tracker : FacepunchBehaviour
        {
            ...
            void Awake()
            {
                ...
                InvokeRepeating(new Action(Track), 0f, 0.5f);
            }
        ...
        public class Rematch
        {
            ...
            public void Notify()
            {
                MessageAll("RematchNotify", 60f, match == null ? ins.szDuelChatCommand : ins.szMatchChatCommand);
            ...
            private void Cancel()
            {
                ...
                {
                    if (ins.sendHomeSpectatorWhenRematchTimesOut)
                    {
                        foreach (var player in Players)
                        {
            ...
            public void Start()
            {
                if (match == null)
                {
                    ...
                    {
                        player.ChatMessage(ins.msg("AllZonesFull", player.UserIDString, duelingZones.Count, ins.playersPerZone));
                        target.ChatMessage(ins.msg("AllZonesFull", target.UserIDString, duelingZones.Count, ins.playersPerZone));
                    }
                }
                else
                {
        ...
        public class GoodVersusEvilMatch
        {
            ...
            public void Setup(BasePlayer player, BasePlayer target)
            {
                ...
                if (_teamSize < ins.minDeathmatchSize)
                    _teamSize = ins.minDeathmatchSize;
            ...
            {
                Item item = ItemManager.CreateByName(ins.teamShirt, 1, GetTeam(player) == Team.Evil ? ins.teamEvilShirt : ins.teamGoodShirt);
                    ...
                    case Team.Evil:
                        {
                            ...
                            {
                                AwardPlayer(playerId, ins.teamEconomicsMoney, ins.teamServerRewardsPoints);
            ...
            private void EndMatch(Team team)
            {
                if (!_ended && _started)
                {
                    ...
                    {
                        if (ins.guiAnnounceUITime > 0f && (_goodKIA.Contains(target.userID) || _evilKIA.Contains(target.userID)))
            ...
            private void Queue()
            {
                ...
                var zones = duelingZones.Where(x => x.TotalPlayers == 0 && !x.IsLocked && x.Spawns.Count >= (ins.requireTeamSize ? TeamSize * 2 : 2)).ToList();
            ...
            private void Spawn(HashSet<BasePlayer> players, Vector3 spawn)
            {
                foreach (var player in players)
                {
                    ...
                    if (ins.immunityTime >= 1)
                    {
                        dataImmunity[player.UserIDString] = TimeStamp() + ins.immunityTime;
        ...
        {   
            ...
            public bool IsFull
            {
                get
                {
                    return TotalPlayers + _waiting.Count + 2 > ins.playersPerZone || IsLocked;
                }
            }
            ...
            {
                var e = col?.ToBaseEntity();
                var m = e as BaseMountable;

                if (m.IsValid())
                {
                    EjectMountable(m, m is MiniCopter || m is CH47Helicopter || m is BaseBoat || m is StaticInstrument);
                    return;
                }
            ...
            private void EjectMountable(BaseMountable m, bool flag)
            {
                ...
                var position = ((m.transform.position.XZ3D() - _zonePos.XZ3D()).normalized * (ins.zoneRadius + 10f)) + _zonePos;
            ...
            public bool RemovePlayer(BasePlayer player)
            {
                ...
                if (RemoveMountable(player.GetMounted()))
                {
                    return true;
                }
            ...
            {
                var position = ((a.XZ3D() - _zonePos.XZ3D()).normalized * (ins.zoneRadius + distance)) + _zonePos; // credits ZoneManager
                float y = TerrainMeta.HighestPoint.y + 250f;
            ...
            private void CreateSpheres()
            {
                ...
                for (int i = 0; i < ins.sphereAmount; i++)
                {
                    ...
                    sphere.Spawn();
                    sphere.LerpRadiusTo(ins.zoneRadius * 2f, ins.zoneRadius * 0.75f);
                    spheres.Add(sphere);
                }
            }
            ...
            public bool? AddWaiting(BasePlayer player, BasePlayer target)
            {
                ...
                double requiredDuelMoney = ins.requiredDuelMoney;

                if (requiredDuelMoney > 0.0 && ins.Economics != null)
                {
                    double playerMoney = Convert.ToDouble(ins.Economics.Call("Balance", player.userID));
                    double targetMoney = Convert.ToDouble(ins.Economics.Call("Balance", target.userID));

                    if (playerMoney < ins.requiredDuelMoney || targetMoney < requiredDuelMoney)
                    {
                        RemoveFromQueue(player.UserIDString);
            ...
            public void Kill()
            {
                ...
                {
                    if (Distance(player.transform.position) <= ins.zoneRadius)
                    {
                        EndSpectate(player);
                        ins.SendHome(player);
                    }
        ...
        private void OnServerInitialized()
        {
            ...
            {
                if (duelsData.VictoriesSeed.Count > 0 && duelsData.VictoriesSeed.Values.Any(x => x > 0))
                {
                    resetDuelists = true;
                }
            }
        ...
        {
            if (player == null || target == null || player == target || visibleToAdmins && target.IsAdmin) // 0.1.3 fix: check if player is null
                return null;
        ...
        {
            return CanNetworkTo(heldEntity?.GetOwnerPlayer(), target); // 0.1.3 fix: check if player is null
        }*/
            ...
            if (IsDueling(player))
            {
                ...
                foreach (var zone in duelingZones)
                {
                    if (zone.IsWaiting(player))
                    {
                        ...
                        //if (useInvisibility) Disappear(player);
        ...
        {
            if (Convert.ToBoolean(AimTrain?.Call("IsAimTraining", player.userID)))
            {
        ...
        public static void SetupPower(BaseEntity e)
        {
            if (ins.autoOvens && e is BaseOven)
            {
                e.SetFlag(BaseEntity.Flags.On, true);
            }
            else if (ins.autoFlames && e is FlameTurret)
            {
            ...
            else if (ins.autoTurrets && e is AutoTurret)
            {
        ...
        private void OnDuelistLost(BasePlayer victim, bool sendHome)
        {
            ...
            var attacker = BasePlayerFind(attackerId);
        ...
        public void SendDuelistsHome()
        {
            foreach (var entry in dataDuelists.ToList())
            {
                ...
                {
                    var target = BasePlayerFind(entry.Key);
        ...
        private static void EndSpectate(BasePlayer player)
        {
            ...
            {
                if (ins.playerHealth > 0f && player.IsAlive())
                    player.health = ins.playerHealth;
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitInfo)
        {
            ...
            if (hitInfo == null || hitInfo.Initiator == null || hitInfo.Initiator.IsDestroyed || hitInfo.Initiator.transform == null || !hitInfo.hasDamage)
                return null;
            ...
            if (adt && entity is BaseHelicopter)
                return true;
            ...
            if ((adt || vdt) && hitInfo.Initiator != null && !hitInfo.Initiator.IsDestroyed && hitInfo.Initiator.IsNpc) // 1.2.0
            {
                ...
                else if (hitInfo.Initiator is BasePlayer)
                {
                    ...
                    if (npc != null)
                    {
                        if (npc is Scientist)
                        {
                            (npc as Scientist).LootSpawnSlots = new LootContainer.LootSpawnSlot[0];
                        }
                        else if (npc is NPCMurderer)
                        {
        ...
        void OnEntitySpawned(WorldItem worldItem)
        {
            NextTick(() =>
            {
                if (worldItem != null && !worldItem.IsDestroyed && worldItem.item != null && worldItem.item.info.shortname == "arrow.bone")
                {
        ...
        {
            if (duelsData.Queued.ContainsValue(player.UserIDString) || InEvent(player) || DuelTerritory(player.transform.position))
            {
                return false;
            }
        ...
        {
            if (!init || !player.IsValid() || player.transform == null)
            {
                return null;
            }
                ...
                case "kickban":
                    {
                        if (deathmatch != null)
                        {
                            ...
                            if (args.Length == 2)
                            {
                                var target = BasePlayerFind(args[1]);
                ...
                case "decline":
                    {
                        ...
                        else // also handle cancelling a match request
                        {
                            ...
                            {
                                var target = BasePlayerFind(tdmRequests[player.UserIDString]);
                ...
                case "accept":
                    {
                        ...
                        var target = BasePlayerFind(kvp.Key);
                ...
                default:
                    {
                        ...
                        var target = BasePlayerFind(args[0]);
        ...
        {
            if (!init || !player || !player.IsConnected || Convert.ToBoolean(AimTrain?.Call("IsAimTraining", player.userID)))
                return;
        ...
        {
            if (!init || Convert.ToBoolean(AimTrain?.Call("IsAimTraining", player.userID)))
                return;
                ...
                case "me":
                    {
                        if (player.IsAdmin)
                        {
                            ...
                            if (Physics.Raycast(player.eyes.HeadRay(), out hit, Mathf.Infinity, wallMask))
                            {
                                ...
                                if (zoneCounter > 0) player.ChatMessage($"Zone will reset after {zoneCounter} duels. Disable by setting `Create New Zone Every X Duels [0 = disabled]` to `0` in the config.");
                            }
                            else
                ...
                case "ban":
                    {
                        ...
                        {
                            string targetId = args[1].IsSteamId() ? args[1] : BasePlayerFind(args[1])?.UserIDString ?? null;
                ...
                case "bet":
                    {
                        if (!allowBets)
                        {
                            player.ChatMessage("Betting is disabled.");
                            break;
                        }
                ...
                case "decline":
                    {
                        ...
                        {
                            var target = BasePlayerFind(entry.Key) ?? BasePlayerFind(entry.Value);
        ...
        public BasePlayer BasePlayerFind(string value)
        {
            foreach (var x in BasePlayer.activePlayerList)
            {
                if (x.UserIDString == value || x.displayName.Contains(value, System.Globalization.CompareOptions.OrdinalIgnoreCase))
                {
                    return x;
                }
            }
        ...
        public void RemoveDuelZone(DuelingZone zone)
        {
            ...
            {
                var player = BasePlayerFind(playerId);
        ...
        {
            return duelingZones.FirstOrDefault(zone => zone.Distance(startPos) <= ins.zoneRadius + offset);
        }
        ...
        public void GiveRespawnLoot(BasePlayer player)
        {
            ...
            else if (!string.IsNullOrEmpty(autoKitName) && Kits != null && IsKit(autoKitName))
            {
                player.inventory.Strip();
        ...
        private void CheckDuelistMortality()
        {
            ...
            if (dataImmunity.Count > 0) // each player that spawns into a dueling zone is given immunity for X seconds. here we'll keep track of this and remove their immunities
            {
                ...
                {
                    var player = BasePlayerFind(kvp.Key);
        ...
        public void SubscribeHooks(bool flag) // we're using lots of temporary and permanent hooks so we'll turn off the temporary hooks when the plugin is loaded, and unsubscribe to others inside of their hooks when they're no longer in use
        {
            ...
            //if (useInvisibility) Subscribe(nameof(CanNetworkTo));
        ...
        public static bool DuelTerritory(Vector3 position, float offset = 5f) // 0.1.21: arena can be inside of the zone at any height
        {
            return init && duelingZones.Any(zone => Vector3Ex.Distance2D(zone.Position, position) <= ins.zoneRadius + offset);
        }
        ...
        {
            return player != null && (dataDuelists.ContainsKey(player.UserIDString) || tdmMatches.Any(match => match.GetTeam(player) != Team.None));
        }

        public static bool IsDueling(BasePlayer player)
        {
            return init && player != null && duelsData != null && duelingZones.Count > 0 && player != null && dataDuelists.ContainsKey(player.UserIDString) && DuelTerritory(player.transform.position);
        }

        public static bool InDeathmatch(BasePlayer player)
        {
            return init && player != null && tdmMatches.Any(team => team.GetTeam(player) != Team.None) && DuelTerritory(player.transform.position);
        }

        public static bool IsSpectator(BasePlayer player)
        {
            return init && player != null && spectators.Contains(player.UserIDString) && DuelTerritory(player.transform.position);
        }
        ...
        public void CheckQueue()
        {
            ...
            var player = BasePlayerFind(playerId);
            var target = BasePlayerFind(targetId);
        ...
        public bool IsAllied(BasePlayer player, BasePlayer target)
        {
            ...
            return IsOnSameTeam(player.userID, target.userID) || IsInSameClan(player, target) || IsAuthorizing(player, target) || IsBunked(player, target) || IsCodeAuthed(player, target) || IsInSameBase(player, target);
        }
        ...
        public bool IsInSameClan(BasePlayer player, BasePlayer target) // 1st method.
        {
            ...
            string playerClan = Clans?.Call("GetClanOf", player.UserIDString) as string;
            ...
            string targetClan = Clans?.Call("GetClanOf", target.UserIDString) as string;
        ...
        public void GivePlayerKit(BasePlayer player)
        {
            ...
            if (!string.IsNullOrEmpty(kit))
            {
                if (Kits != null && IsKit(kit))
                {
                    object success = ins.Kits.Call("GiveKit", player, kit);
        ...
        private void DuelAnnouncement(bool bypass)
        {
            ...
            foreach (var player in BasePlayer.activePlayerList.Where(p => p?.displayName != null))
            {
                ...
                player.ChatMessage(string.Format("{0} <color=#C0C0C0>{1}</color>", lang.GetMessage("Prefix", this, player.UserIDString), message));
            }
        }
        ...
        {
            return duelsData.Spawns.Select(entry => entry.ToVector3()).Where(spawn => zone.Distance(spawn) < ins.zoneRadius).ToList();
        }
        ...
        {
            if (!player || !player.IsConnected || ins.guiAnnounceUITime <= 0f)
                return;
            ...
            ins.timer.Once(ins.guiAnnounceUITime, () => CuiHelper.DestroyUi(player, "DuelistUI_Announcement"));
        }
        ...
        public static void CreateDefeatUI(BasePlayer player)
        {
            ...
            var element = UI.CreateElementContainer("DuelistUI_Defeat", "0 0 0 0.5", "0.436 0.133", "0.534 0.307", ins.guiUseCursor);
        ...
        private void UpdateMatchUI()
        {
            ...
            foreach (string userId in matchesUI.ToList())
            {
                ...
                var player = BasePlayerFind(userId);
