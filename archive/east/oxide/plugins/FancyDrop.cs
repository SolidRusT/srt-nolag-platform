...
namespace Oxide.Plugins
{
    ...
    class FancyDrop : RustPlugin
    {
        ...
        bool useSmokeEffectOnDrop;
        bool useSirenAlarmOnDrop;
        bool useSirenAlarmAtNightOnly;
        bool useSirenLightOnDrop;
        bool useSirenLightAtNightOnly;

        bool shootDownDrops;
        int shootDownCount;
        float dropDragsetting;
        float dropMassSetting;
        float dropAngularDragSetting;
        bool explodeImpact;
        int explodeChance;
        ...
        bool notifyDropServerLootedGrid;
        ...
        void LoadVariables()
        {
            ...
            useSmokeEffectOnDrop = Convert.ToBoolean(GetConfig("Airdrop", "Deploy Smoke on drop as it falls", true));
            useSirenAlarmOnDrop = Convert.ToBoolean(GetConfig("Airdrop", "Deploy with Audio Alarm on drop", true));
            useSirenAlarmAtNightOnly = Convert.ToBoolean(GetConfig("Airdrop", "Deploy with Audio Alarms on drop only during the night", true));
            useSirenLightOnDrop = Convert.ToBoolean(GetConfig("Airdrop", "Deploy with Spinning Siren Light on drop", true));
            useSirenLightAtNightOnly = Convert.ToBoolean(GetConfig("Airdrop", "Deploy with Spinning Siren Light on drop only during the night", true));
            ...
            Color = Convert.ToString(GetConfig("Generic", "Prefix color", "#00ffff"));
            Format = Convert.ToString(GetConfig("Generic", "Prefix format", "<color={0}>{1}</color>: "));
            guiCommand = Convert.ToString(GetConfig("Generic", "GUI Announce command", "announce"));
            ...
            colorAdmMsg = Convert.ToString(GetConfig("Generic", "Admin messages color", "#c0c0c0"));
            notifyByChatAdminCalls = Convert.ToBoolean(GetConfig("Generic", "Show message to admin after command usage", true));
            colorTextMsg = Convert.ToString(GetConfig("Generic", "Broadcast messages color", "#ffffff"));
            ...
            notifyDropServerSignalGrid = Convert.ToBoolean(GetConfig("Notification", "Notify players at Drop by SupplySignal including Grid Area", false));
            ...
            notifyDropServerRegularGrid = Convert.ToBoolean(GetConfig("Notification", "Notify players at Random/Timed Drop including Grid Area", false));
            ...
            notifyDropServerCustomGrid = Convert.ToBoolean(GetConfig("Notification", "Notify players at custom/event Drop including Grid Area", false));
            ...
            notifyDropServerLootedGrid = Convert.ToBoolean(GetConfig("Notification", "Notify players when a Drop is being looted including Grid Area", false));
            ...
            notifyDropSignalByPlayerGrid = Convert.ToBoolean(GetConfig("Notification", "Notify Players who has thrown a SupplySignal including Grid Area", false));
            ...
            shootDownDrops = Convert.ToBoolean(GetConfig("Allow Damage Airdrop Settings", "Players can shoot down the drop", false));
            shootDownCount = Convert.ToInt32(GetConfig("Allow Damage Airdrop Settings", "Players can shoot down the drop - needed hits", 3));
            dropDragsetting = Convert.ToSingle(GetConfig("Allow Damage Airdrop Settings", "Set Drag for drop (drop resistance)", 0.3f));
            dropMassSetting = Convert.ToSingle(GetConfig("Allow Damage Airdrop Settings", "Set Mass weight for drop", 0.75f));
            dropAngularDragSetting = Convert.ToSingle(GetConfig("Allow Damage Airdrop Settings", "Set Angular Drag for drop", 0.1f));
            explodeImpact = Convert.ToBoolean(GetConfig("Allow Damage Airdrop Settings", "Set drop allow to explode on impact", true));
            explodeChance = Convert.ToInt32(GetConfig("Allow Damage Airdrop Settings", "Set drop chance exploding on impact (x out of 100)", 25));
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity is SupplyDrop)
            {
                ...
                parachute.Kill();
                parachute = null;
                drop.GetComponent<Rigidbody>().drag = dropDragsetting;
                drop.GetComponent<Rigidbody>().mass = dropMassSetting;
                ...
                drop.GetComponent<Rigidbody>().angularDrag = dropAngularDragSetting;
        ...
        sealed class ColliderCheck : FacepunchBehaviour
        {
            ...
            void OnCollisionEnter(Collision col)
            {
                if (!landed)
                {
                    ...
                    if (wasHit)
                    {
                        ...
                        if (fd.explodeImpact)
                        {
                            if (UnityEngine.Random.Range(0, 100) <= fd.explodeChance)
                            {
            ...
            IEnumerator HitRemove(bool explode)
            {
                if (explode)
                {
            ...
            IEnumerator DeSpawn()
            {
                yield return new WaitForSeconds(Convert.ToSingle((int)cratesettings["despawnMinutes"]) * 60);
        ...
        void CreateDropEffects(BaseEntity entity)
        {
            if (entity == null) return;

            if (useSirenAlarmOnDrop && useSirenAlarmAtNightOnly && TOD_Sky.Instance.IsNight)
                CreateSirenAlarms(entity);
            else if (useSirenAlarmAtNightOnly && TOD_Sky.Instance.IsNight)
                CreateSirenAlarms(entity);
            else if (useSirenAlarmOnDrop)
                CreateSirenAlarms(entity);
            else
                return;

            if (useSirenLightOnDrop && useSirenLightAtNightOnly && TOD_Sky.Instance.IsNight)
                CreateSirenLights(entity);
            else if (useSirenLightAtNightOnly && TOD_Sky.Instance.IsNight)
                CreateSirenLights(entity);
            else if (useSirenLightOnDrop)
                CreateSirenLights(entity);
            else
                return;
        }
        ...
        void createSupplyDrop(Vector3 pos, Dictionary<string, object> cratesettings, bool notify = true, bool notifyConsole = true, Vector3 start = new Vector3(), Vector3 end = new Vector3())
        {
            ...
            if (useSmokeEffectOnDrop)
                RunEffect(SMOKE_EFFECT, newDrop);

            if (useSirenAlarmOnDrop || useSirenLightOnDrop && newDrop != null)
            {
            ...
            else if (dropType == "supplysignal")
            {
                if (notifyDropServerSignal)
                    if (notifyDropGUI && SimpleUI_Enable)
                        ...
                        else if (notifyDropServerSignalGrid)
                            foreach (var player in BasePlayer.activePlayerList)
                                MessageToPlayerUI(player, string.Format(lang.GetMessage("msgDropSignalGrid", this, player.UserIDString), gridPos));
                        else
                            foreach (var player in BasePlayer.activePlayerList)
                        ...
                        if (notifyDropServerSignalCoords)
                            ...
                            MessageToAllGui(string.Format(lang.GetMessage("msgDropSignalGrid", this), gridPos));
                        else
                        ...
                        foreach (var player in BasePlayer.activePlayerList)
                            MessageToPlayer(player, string.Format(lang.GetMessage("msgDropSignalGrid", this, player.UserIDString), gridPos));
                    else
                        foreach (var player in BasePlayer.activePlayerList)
            ...
            else if (dropType == "regular")
            {
                if (notifyDropServerRegular)
                    if (notifyDropGUI && SimpleUI_Enable)
                        ...
                        else if (notifyDropServerRegularGrid)
                            foreach (var player in BasePlayer.activePlayerList)
                                MessageToPlayerUI(player, string.Format(lang.GetMessage("msgDropRegularGrid", this, player.UserIDString), gridPos));
                        else
                            foreach (var player in BasePlayer.activePlayerList)
                        ...
                        if (notifyDropServerRegularCoords)
                            ...
                            MessageToAllGui(string.Format(lang.GetMessage("msgDropRegularGrid", this), gridPos));
                        else
                        ...
                        foreach (var player in BasePlayer.activePlayerList)
                            MessageToPlayer(player, string.Format(lang.GetMessage("msgDropRegularGrid", this, player.UserIDString), gridPos));
                    else
                        foreach (var player in BasePlayer.activePlayerList)
            ...
            else
            {
                if (notifyDropServerCustom)
                    if (notifyDropGUI && SimpleUI_Enable)
                        ...
                        else if (notifyDropServerCustomGrid && _massDropTimer != null && _massDropTimer.Repetitions == 0)
                            foreach (var player in BasePlayer.activePlayerList)
                                MessageToPlayerUI(player, string.Format(lang.GetMessage("msgDropCustomGrid", this, player.UserIDString), notificationInfo, gridPos));
                        else
                            foreach (var player in BasePlayer.activePlayerList)
                        ...
                        if (notifyDropServerCustomCoords && _massDropTimer != null && _massDropTimer.Repetitions == 0)
                            ...
                            MessageToAllGui(string.Format(lang.GetMessage("msgDropCustoGrid", this), notificationInfo, gridPos));
                        else
                        ...
                        foreach (var player in BasePlayer.activePlayerList)
                            MessageToPlayer(player, string.Format(lang.GetMessage("msgDropCustomGrid", this, player.UserIDString), notificationInfo, gridPos));
                    else
                        foreach (var player in BasePlayer.activePlayerList)
        ...
        void SupplyThrown(BasePlayer player, BaseEntity entity)
        {
            ...
            timer.Once(3.3f, () => {
                ...
                if (notifyDropSignalByPlayer)
                    ...
                    else if (notifyDropSignalByPlayerGrid)
                        PrintToChat(string.Format(Format, Color, Prefix) + $"<color={colorTextMsg}>" + string.Format(lang.GetMessage("msgDropSignalByPlayerGrid", this, player.UserIDString), player.displayName, gridPos) + "</color>");
                    else
        ...
        void OnLootEntity(BasePlayer player, BaseEntity entity)
        {
            ...
            if (Vector3.Distance(lastLootPos, entity.transform.position) > ((lastDropRadius * 2) * 1.2))
            {
                ...
                if (entity.GetComponent<ColliderCheck>() != null && entity.GetComponent<ColliderCheck>().cratesettings != null && Convert.ToString(entity.GetComponent<ColliderCheck>().cratesettings["droptype"]) == "dropdirect")
                    direct = true;
                if (notifyDropServerLooted && !direct)
        ...
        void cdropRandom(BasePlayer player, string command)
        {
            ...
            SendReply(player, $"<color={colorAdmMsg}>Random Airdrop incoming at: {newpos.ToString("0")} grid area: {gridPos}</color>");
            if (airdropTimerEnabled && airdropTimerResetAfterRandom)
                airdropTimerNext();
        }
        ...
        void cdropToPos(BasePlayer player, string command, string[] args)
        {
            ...
            if (notifyByChatAdminCalls)
                SendReply(player, $"<color={colorAdmMsg}>Airdrop called to position: {newpos.ToString("0")} grid area: {gridPos}</color>");
        }
        ...
        void cdropToPlayer(BasePlayer player, string command, string[] args)
        {
            ...
            else
            {
                ...
                startCargoPlane(newpos, false, null, "dropdirect");
                if (notifyByChatAdminCalls)
                    SendReply(player, $"<color={colorAdmMsg}>Airdrop called to player '{target.displayName}' at: {newpos.ToString("0")} grid area: {gridPos}</color>");
                if (notifyDropPlayer)
                    MessageToPlayer(target, lang.GetMessage("msgDropPlayer", this, target.UserIDString));
            }
        }
        ...
        void NotifyOnDropLooted(BaseEntity drop, BasePlayer looter)
        {
            ...
            foreach (var player in BasePlayer.activePlayerList.Where(p => Vector3.Distance(p.transform.position, drop.transform.position) < supplyLootNotifyDistance).ToList())
                ...
                else if (notifyDropServerLootedGrid)
                    MessageToPlayer(player, string.Format(lang.GetMessage("msgDropLootetGrid", this, player.UserIDString), looter.displayName, gridPos));
                else
        ...
        {
            if (fd.AlphaLoot != null && fd.AlphaLoot.Call("WantsToHandleFancyDropLoot") != null)
                return;

            if (fd.MagicLoot != null && fd.MagicLoot.CallHook("OnLootSpawn", new object[] { drop.GetComponent<LootContainer>() }) != null)
                return;
