...
{
    [Info("Human NPC", "Ts3Hosting", "0.3.47")]
...
//                    npc.Invoke("AllowMove",0);
            ...
            public void CreateProjectileEffect(BaseCombatEntity target, BaseProjectile baseProjectile, float dmg, bool miss = false)
            {
				...
				{					
					...
					if (!miss) ApplyDamage(target, target.transform.position, vector32, target);
...
//                if (IsInvoking("AllowMove")) { CancelInvoke("AllowMove"); AllowMove(); }
        ...
        private void Init()
        {
			...
			CheckCfg("Place Npc in teams", ref TeamsNPC);
            SaveConfig();
            LoadData();
        }
        ...
        private void Unload()
        {
			...
			{
				RelationshipManager.Instance.DisbandTeam(team.Value);
			}
        }
        ...
        private void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitinfo)
        {
            ...
            if(humanPlayer != null)
            {
				...
				if (hitinfo.Initiator.ToString().Contains("fireball") && humanPlayer.info.invulnerability)
				{
		...
		private void AddNpcTeam(BasePlayer player, BasePlayer teamPlayer)
		{
			if (teamPlayer.currentTeam != null && teamPlayer.currentTeam != 0UL)
			{
				...
				RelationshipManager.PlayerTeam team = RelationshipManager.Instance.teams[teamPlayer.currentTeam];
				RelationshipManager.PlayerTeam playerTeam = team;
					if (!playerTeam.AddPlayer(player))
					{
						player.currentTeam = playerTeam.teamID;
